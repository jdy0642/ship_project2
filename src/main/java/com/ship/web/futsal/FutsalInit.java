	...
	public void run(ApplicationArguments args) throws Exception {
		...
		if(count < 3000) {
			...
			for(int i = 0; i<=1000; i++) {
				...
				match.setNum(random(4, 7));
				match.setGender(Arrays.asList("male","female","male").get(random(1,3)));
				match.setDifficulty(random(1, 4));
				match.setShoes("shoes0");
				match.setStadiumfacility("size0,shower0,park0,shoes0,wear0");
